...
previewed (as a file list) and undoable via the one *Undo last action* command.
...
- [ ] **Undo last action** → the files move back to root; embeds still resolve.
...
- [ ] Delete → they go to trash. **Undo last action** restores them.
...
- [ ] Attachments-move and empty-delete both undo in one step via the same
      *Undo last action* command as split/merge/scaffold (shared undo stack).
